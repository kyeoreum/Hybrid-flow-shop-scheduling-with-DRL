...
def get_feature(stage_queue, machine_state2):
    ...
    acr = 0
    for i in range(0, len(completion_time)):
...
def act(cur_state):
    ...
    cur_state = cur_state.reshape(-1, 8*1)
...
def replay(batch_size):
    ...
    for state, action, reward, next_state, done in minibatch:
        ...
        state = state.reshape(-1, 8*1)

        next_state = np.array(next_state)
        next_state = next_state.reshape(-1, 8*1)
    ...
    for sn in stage:
        for al in alternative_machine:
            for num in range(0, 10):
                if num % 3 == 0:
                    target_train()
                f = open("C:/Users/user/source/repos/my_thesis/TESTData/TESTData_"+str(jn)+"_"+str(sn)+"_"+str(al)+"_"+str(num)+".txt", 'r')                            
                Done = False
                ##### Data Preprocessing #####
                ...
                while check != job_num:                        
                    training_period += 1
                    if training_period % 3 == 0 and len(memory) > batch_size:
                        print(len(memory))
                        replay(batch_size)
                        ...
                        while j < machine_num:
                            ...
                            if machine_state[i][j] != 9999:
                                if job_temp[machine_state[i][j]][3][i] != 0:
                                    ...
                                    cnt_len = 0
                                    for cnt in range(0, job_num):
                                        cnt_len += len(completion_time[cnt])
                                    if cnt_len == job_num * stage_num:
                                        Done = True
