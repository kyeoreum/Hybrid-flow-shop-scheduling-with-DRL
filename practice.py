...
def get_feature():
    ...
    for n in range(0, job_num):
        if job_flow[n] > job_temp[n][-1][0]:
            at += (stage_num * job_num) - len(completion_time[n])
    if at == 0:
        at = 0
    else:
        at /= left
    ...
    for i in range(0, job_num):
        temp += len(completion_time[n])
    if temp < stage_num * job_num:
        left += left + (stage_num * job_num) - temp
    else:
        left = 1

    for i in range(0, job_num):    
        for j in range(temp+1, stage_num):
            T_left = T_left + job_temp[i][3][j]
    ...
    left = 0
    for i in range(0, job_num):
        temp = len(completion_time[i])
        for j in range(temp+1, stage_num):
            left += job_temp[i][3][j]
        if left == 0:
            left = 1
        temp2 = (job_temp[i][-1][0] - job_flow[i])/left
        if temp2 <= 0:
            cr += 1
    cr /= job_num
    features.append(cr)
...
def create_model():
    ...
    model = Sequential()
    model.add(LSTM(128, batch_input_shape = (None,6,1), return_sequences=True, activation = 'tanh'))
    model.add(Dense(64))
...
def act(cur_state):
    ...
    cur_state = cur_state.reshape(1,6,1)
...
def replay(batch_size):
    ...
    for state, action, reward, next_state in minibatch:
        state = state.reshape(1,6,1)
        next_state = next_state.reshape(1,6,1)
        target = reward
        target = (reward + gamma *
    ...
    for sn in stage:
        for al in alternative_machine:
            for num in range(0, 10):
                ...
                f = open("C:/Users/user/source/repos/my_thesis/Data/TData_"+str(jn)+"_"+str(sn)+"_"+str(al)+"_"+str(num)+".txt", 'r')                            
